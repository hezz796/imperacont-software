...
- **Data:** 2026-09-22
- **Status:** Revisado (revisão independente P1–P13 na v1.1 e revisão da aplicação das respostas na v1.2)
...
| RF-003 | Lançar movimentações financeiras de um cliente (data, valor, natureza/débito-crédito, período, descrição) — **modelo unitário** (uma linha por movimento), confirmado como direção provisória (RESUMO-001 A1; validação externa pendente) | Dor 1: erros por ausência de validação | Alta | PB-001 §11.1, EVD-014, RESUMO-001 A1 | Draft — direção provisória |
| RF-004 | Validar lançamentos **no momento da confirmação** conforme política por tipo de inconsistência: duplicidade e período = **C (confirmar antes de gravar** — avisa, pergunta; "manter mesmo assim" grava sinalizado p/ conferência); balanceamento = N/A (modelo unitário) — política provisória (RESUMO-001 A2) | Dor 1: retrabalho; conserta a ênfase em "sinalizar" (RSK-PB-002) | Alta | PB-001 §11.1, BE-01, HYP-005, RESUMO-001 A2 | Draft — direção provisória |
...
| RF-006 | Controlar prazos/obrigações por cliente (cadastrar obrigação, data de vencimento, status, responsável). Modelo: **catálogo configurável de obrigações, padrão por regime** aplicado ao cadastrar cliente com ajuste por cliente; **responsável típico por entrega = contador titular**; **lista inicial sugerida (insumo provisório): DAS, DCTF Web, IRPJ, CSLL, PIS, COFINS** — lista definitiva é do contador (PEN-PB-004/Q8/Q12) (RESUMO-001 B1/B5) | Dor 5: risco de perder prazos | Alta | PB-001 §11.1, EVD-014, BE-03, RESUMO-001 B1/B5 | Draft — direção provisória |
| RF-007 | Gerar/atualizar status do prazo automaticamente (em dia, a vencer, atrasado, **entregue**) com registro de entrega (**data + autor obrigatórios; anexo opcional**) e alertar vencimento próximo (**padrão 7 dias antes**, configurável) — RESUMO-001 B2/B4 | Dor 5; confiabilidade dos alertas (RSK-PB-008) | Alta | PB-001 §11.1, RSK-PB-008; RN-006; RESUMO-001 B2/B4 | Draft — direção provisória |
...
| RN-003 | Duplicidade | Lançamento com **mesmo cliente + mesma data + mesmo valor** já registrado (definição provisória — RESUMO-001 A3) | **Política C — confirmar antes de gravar**: avisa "Já existe lançamento parecido"; [Corrigir] [Manter mesmo assim]; manter grava sinalizado p/ conferência (RN-005) | RESUMO-001 A2a/A3 (direção provisória); PB-001 §11.1 |
| RN-004 | Período/data incorreto | **Data do lançamento ≠ período selecionado** (definição provisória — RESUMO-001 A4) | **Política C — confirmar antes de gravar**: avisa; usuário corrige ou mantém sinalizado | RESUMO-001 A2c/A4 (direção provisória); PB-001 §11.1 |
...
| PEN-PB-002 | Regras exatas de validação + **golden cases aprovados** (Q6/Q12 do PEN-CONTADOR-001) — direção de política já consolidadas como provisória (RESUMO-001 A) | Contador | Alta | Development |
| PEN-PB-003 | Validação de fluxo com auxiliares e 1 cliente — **UX pode iniciar exploração, mas NÃO fecha telas/jornadas antes desta pendência** | UX | Alta | Development (UX não fecha antes) |
...
> 3. **Antes de Development:** permanece bloqueado até golden cases (PEN-PB-002) e validação com usuários (PEN-PB-003).
>
